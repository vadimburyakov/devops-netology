...
```TEXT
Возможно, память занята истекшими одновременно ключами, которые еще не удаленны. 
Так как Redis в основном является однопоточным, то все запросы обслуживаются 
последовательно, поэтому пока не будет произведена очистка, Redis блокирует 
операции записи.
```
### Задача 3 (MySQL)
...
```TEXT
Можно изменить параметры, которые регулируют память в Postgres, такие как: max_connections, shared_buffer, 
wal_buffers, work_mem, effective_cache_size, maintenance_work_mem.

shared_buffer - параметр определяет, сколько выделенной памяти использует PostgreSQL для кеширования.
wal_buffers - по-умолчанию wal_buffers равен 16 МБ. Если много одновременных подключений,
то повысив его значение, можно увеличить производительность.
effective_cache_size - предоставляет оценку памяти, доступной для кэширования диска. Это ориентир, 
есть смысл установить значение побольше, это даст возможность оптимизатору использовать нужные
индексы.
work_mem - в случае необходимости выполнения большой сортировки увеличение параметра даст 
положительный результат, так как сортировка будет проводиться в памяти, а не на диске.
maintenance_work_mem - это параметр памяти, который используется для задач обслуживания.
```
...
```TEXT
Будем использовать не изменяемый тип инфраструктуры, на основе образов, созданных с помощью Packer.
```
Будет ли центральный сервер для управления инфраструктурой?
```TEXT
Нет, так как будем использовать не требующие сервера инструменты: Packer, Terraform, Ansible.
При этом код инфраструктуры будет храниться на сервере - git.
```
...
```
### Задача 3
Скачан и распакован бинарник версии 1.0.11
```bash
vagrant@vagrant:~$ terraform --version
Terraform v1.1.5
on linux_amd64
vagrant@vagrant:~$ terraform_011 --version
Terraform v1.0.11
on linux_amd64
```
...
#### Crontab настроен на запуск вышеуказанного скрипта в 7:54(+3:00), 28-го числа каждого месяца.
![](screenshots/curs5.png)
#### После запуска скрипта в 7:54(+3:00), 28.07.2022 файлы `/etc/nginx/serv01.example.crt` и `/etc/nginx/serv01.example.key` обновились,
![](screenshots/curs6.png)
#### nginx рестартовал,
![](screenshots/curs7.png)
#### страница сервера nginx в браузере хоста не содержит предупреждений.
![](screenshots/curs8.png)
